...
class AnonymouseConnectivityController : NSObject, MCNearbyServiceAdvertiserDelegate, MCNearbyServiceBrowserDelegate, MCSessionDelegate {
    ...
    var hasReceivedAllMessages = false;
    //MARK: Links
    ...
    func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        ...
        if state == MCSessionState.connected {
            let sortedRequesters = toRequesters
            sortedRequesters.append(myPeerId)
            sortedRequesters = sorted(sortedRequesters, sortFunc)
            for peerId in sortedRequesters {
                if(peerId == myPeerId){
                    self.sendFilteredMessages(toRequesters: [peerID])
                }
                else {
                    while (!hasReceivedAllMessages) {
